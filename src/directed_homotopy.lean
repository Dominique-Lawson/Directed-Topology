...
open directed_map directed_unit_interval auxiliary
...
variables [directed_space X] [directed_space Y] [directed_space Z]

/-- `directed_map.dihomotopy f₀ f₁` is the type of directed homotopies from `f₀` to `f₁`. -/
structure dihomotopy (f₀ f₁ : directed_map X Y) extends D((I × X), Y) :=
  (map_zero_left' : ∀ x, to_fun (0, x) = f₀.to_fun x)
  (map_one_left' : ∀ x, to_fun (1, x) = f₁.to_fun x)

section 
set_option old_structure_cmd true

/-- `directed_map.dihomotopy_class F f₀ f₁` states that `F` is a type of homotopies between `f₀` and `f₁` -/
class dihomotopy_class (F : Type*) (f₀ f₁ : out_param $ D(X, Y))
  extends directed_map_class F (I × X) Y :=
(map_zero_left (f : F) : ∀ x, f (0, x) = f₀.to_fun x)
(map_one_left (f : F) : ∀x, f (1, x) = f₁.to_fun x)

end

/- `directed_map.dihomotopy` namespace -/
